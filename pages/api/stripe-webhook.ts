  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      try {
        ...
        // 🎯 핵심 수정 — 주소 제출 필요 없는 번호만 선택하도록 구현
        const available = await client.availablePhoneNumbers('US').local.list({
          voiceEnabled: true,
          limit: 20,      // 여러 개 검색해놓고 필터링
        });
        ...
        if (purchasable.length === 0) {
          throw new Error('No purchasable phone numbers (address requirement issue)');
        }
...
const smsUrl = 'https://youngchun-mvp.vercel.app/api/twilio-callback';
        ...
        console.log(`✅ Twilio number purchased: ${purchased.phoneNumber} (${purchased.sid})`);
